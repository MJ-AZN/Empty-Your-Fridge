...
function getMealId() {
  ...
  function fetchEdamam(mealTitle, ingredients) {
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      const amount = ingredients[i - 1].amount; // Access amount from the provided ingredients array
      const ingredient = ingredients[i - 1].ingredient; // Access ingredient from the provided ingredients array
  
      // Check if both amount and ingredient are defined and not empty
      if (amount && ingredient) {
        ingredientsArray.push({ amount, ingredient });
      }
    }
}
